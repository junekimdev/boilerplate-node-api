...
import uploader, { getExt } from '../../../src/middleware/uploadImageProfile';
...
describe('Test /src/middleware/uploadImageOnce', () => {
  ...
  describe('uploader', () => {
    ...
    it('should init busboy', async () => {
      ...
      expect(mockedBusboy.mock.calls[0][0].limits.files).toBe(1);
    ...
    describe('"finish" event callback', () => {
      it('should remove file from storage when error occurred during uploading', async () => {
        ...
        mockedPathJoin.mockReturnValue(filename);
        ...
        expect(fs.promises.rm).toBeCalledTimes(1);
        expect(fs.promises.rm).toBeCalledWith(filename, { force: true });
      });
      ...
      it('should set uploadedImagePaths in res.locals when no error occurs', async () => {
        ...
        expect(res.locals.uploadedImagePaths).toHaveLength(1);
      });
    });
