...
- API documentation standard: openapi 3.x
- Database: PostgreSQL
...
- Token signing algorithm: ES256, which is ECDSA using P-256 and SHA256 (curve:secp256k1)
...
1. `/src/api/basicAuth.ts`is the basic authorization middleware
   - This sets `userId` and `email` in `req.locals`
1. `/src/api/bearerAuth.ts`is the bearer authorization middleware
